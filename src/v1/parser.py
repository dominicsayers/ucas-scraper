...
class Parser:
    ...
    def __init__(
        ...
        self.soup = self._create_soup(response)

    def _create_soup(self, response: Optional[Union[bytes, str]]) -> BeautifulSoup:
        """
    ...
    def select(self, selector: str) -> ResultSet[Tag] | list[str]:
        """
...
class ParserContent:
    ...
    def get_content_from(
        ...
        try:
            ...
            return self._extract_content(selected[0], content_type)
        except IndexError:
            return self.config.default_value
        except Exception as e:
...
def create_parser(html_content: Union[bytes, str]) -> Parser:
    """
